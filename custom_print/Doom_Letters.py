# +-----------------------------------------------------------------------------------------------+
# |                                 Doom_Letters                                                  |
# +-----------------------------------------------------------------------------------------------+
#    Uppercase :    A B C D E F G H I J K L M N Ñ O P Q R S T U V W X Y Z                        
#                                                                                                  
#    Lowercase :    a b c d e f g h i j k l m n ñ o p q r s t u v w x y z                        
#                                                                                                  
#    Shift_Off :    ` 1 2 3 4 5 6 7 8 9 0 - = \ [ ] ; ' , . /                                      
#                                                                                                  
#    Shift_On  :    ~ ! @ # $ % ^ & * ( ) _ + | { } : " < > ?                                     
#
#
# +-----------------------------------------------------------------------------------------------+
# |                                   KeyBoard Symbol Names                                       |
# +-----------------------------------------------------------------------------------------------+
# |   ` : Backtick            - : Minus                = : Equal            \ : Backward_Slash    |
# |   [ : Open_Bracket        ] : Close_Bracket        ; : Semicolon        ' : Apostrophe        |
# |   , : Comma               . : Period               / : Forward_Slash    ~ : Tilde             |
# |   ! : Exclamation         @ : Arroba               # : Pound            $ : Dollar            |
# |   % : Percent             ^ : Caret                & : Ampersand        * : Asterisk          |
# |   ( : Open_Parenthesis    ) : Close_Parenthesis    _ : Underscore       + : Plus              |
# |   | : Pipe                { : Open_Curly           } : Close_Curly      : : Colon             |
# |   " : Quotation           < : Less_Than            > : Greater_Than     ? : Question          |
# +-----------------------------------------------------------------------------------------------+

# +--------------------------------+
#   Letter A 
# +--------------------------------+
Doom_A = []
Doom_A.append("       ") # Top, 0
Doom_A.append("  ___  ") # 1
Doom_A.append(" / _ \ ") # 2
Doom_A.append("/ /_\ \\") # 3
Doom_A.append("|  _  |") # 4
Doom_A.append("| | | |") # 5
Doom_A.append("\_| |_/") # 6
Doom_A.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter B 
# +--------------------------------+
Doom_B = []
Doom_B.append("       ") # Top, 0
Doom_B.append("______ ") # 1
Doom_B.append("| ___ \\") # 2
Doom_B.append("| |_/ /") # 3
Doom_B.append("| ___ \\") # 4
Doom_B.append("| |_/ /") # 5
Doom_B.append("\____/ ") # 6
Doom_B.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter C 
# +--------------------------------+
Doom_C = []
Doom_C.append("       ") # Top, 0
Doom_C.append(" _____ ") # 1
Doom_C.append("/  __ \\") # 2
Doom_C.append("| /  \/") # 3
Doom_C.append("| |    ") # 4
Doom_C.append("| \__/\\") # 5
Doom_C.append(" \____/") # 6
Doom_C.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter D 
# +--------------------------------+
Doom_D = []
Doom_D.append("       ") # Top, 0
Doom_D.append("______ ") # 1
Doom_D.append("|  _  \\") # 2
Doom_D.append("| | | |") # 3
Doom_D.append("| | | |") # 4
Doom_D.append("| |/ / ") # 5
Doom_D.append("|___/  ") # 6
Doom_D.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter E 
# +--------------------------------+
Doom_E = []
Doom_E.append("       ") # Top, 0
Doom_E.append(" _____ ") # 1
Doom_E.append("|  ___|") # 2
Doom_E.append("| |__  ") # 3
Doom_E.append("|  __| ") # 4
Doom_E.append("| |___ ") # 5
Doom_E.append("\____/ ") # 6
Doom_E.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter F 
# +--------------------------------+
Doom_F = []
Doom_F.append("       ") # Top, 0
Doom_F.append("______ ") # 1
Doom_F.append("|  ___|") # 2
Doom_F.append("| |_   ") # 3
Doom_F.append("|  _|  ") # 4
Doom_F.append("| |    ") # 5
Doom_F.append("\_|    ") # 6
Doom_F.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter G 
# +--------------------------------+
Doom_G = []
Doom_G.append("       ") # Top, 0
Doom_G.append(" _____ ") # 1
Doom_G.append("|  __ \\") # 2
Doom_G.append("| |  \/") # 3
Doom_G.append("| | __ ") # 4
Doom_G.append("| |_\ \\") # 5
Doom_G.append(" \____/") # 6
Doom_G.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter H 
# +--------------------------------+
Doom_H = []
Doom_H.append("       ") # Top, 0
Doom_H.append(" _   _ ") # 1
Doom_H.append("| | | |") # 2
Doom_H.append("| |_| |") # 3
Doom_H.append("|  _  |") # 4
Doom_H.append("| | | |") # 5
Doom_H.append("\_| |_/") # 6
Doom_H.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter I 
# +--------------------------------+
Doom_I = []
Doom_I.append("       ") # Top, 0
Doom_I.append(" _____ ") # 1
Doom_I.append("|_   _|") # 2
Doom_I.append("  | |  ") # 3
Doom_I.append("  | |  ") # 4
Doom_I.append(" _| |_ ") # 5
Doom_I.append(" \___/ ") # 6
Doom_I.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter J 
# +--------------------------------+
Doom_J = []
Doom_J.append("       ") # Top, 0
Doom_J.append("   ___ ") # 1
Doom_J.append("  |_  |") # 2
Doom_J.append("    | |") # 3
Doom_J.append("    | |") # 4
Doom_J.append("/\__/ /") # 5
Doom_J.append("\____/ ") # 6
Doom_J.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter K 
# +--------------------------------+
Doom_K = []
Doom_K.append("       ") # Top, 0
Doom_K.append(" _   __") # 1
Doom_K.append("| | / /") # 2
Doom_K.append("| |/ / ") # 3
Doom_K.append("|    \ ") # 4
Doom_K.append("| |\  \\") # 5
Doom_K.append("\_| \_/") # 6
Doom_K.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter L 
# +--------------------------------+
Doom_L = []
Doom_L.append("       ") # Top, 0
Doom_L.append(" _     ") # 1
Doom_L.append("| |    ") # 2
Doom_L.append("| |    ") # 3
Doom_L.append("| |    ") # 4
Doom_L.append("| |____") # 5
Doom_L.append("\_____/") # 6
Doom_L.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter M 
# +--------------------------------+
Doom_M = []
Doom_M.append("        ") # Top, 0
Doom_M.append("___  ___") # 1
Doom_M.append("|  \/  |") # 2
Doom_M.append("| .  . |") # 3
Doom_M.append("| |\/| |") # 4
Doom_M.append("| |  | |") # 5
Doom_M.append("\_|  |_/") # 6
Doom_M.append("        ") # Bottom, 7

# +--------------------------------+
#   Letter N 
# +--------------------------------+
Doom_N = []
Doom_N.append("       ") # Top, 0
Doom_N.append(" _   _ ") # 1
Doom_N.append("| \ | |") # 2
Doom_N.append("|  \| |") # 3
Doom_N.append("| . ` |") # 4
Doom_N.append("| |\  |") # 5
Doom_N.append("\_| \_/") # 6
Doom_N.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter Ñ 
# +--------------------------------+
Doom_Ñ = []
Doom_Ñ.append("       ") # Top, 0
Doom_Ñ.append(" ~~~~~ ") # 1
Doom_Ñ.append("| \ | |") # 2
Doom_Ñ.append("|  \| |") # 3
Doom_Ñ.append("| . ` |") # 4
Doom_Ñ.append("| |\  |") # 5
Doom_Ñ.append("\_| \_/") # 6
Doom_Ñ.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter O 
# +--------------------------------+
Doom_O = []
Doom_O.append("       ") # Top, 0
Doom_O.append(" _____ ") # 1
Doom_O.append("|  _  |") # 2
Doom_O.append("| | | |") # 3
Doom_O.append("| | | |") # 4
Doom_O.append("\ \_/ /") # 5
Doom_O.append(" \___/ ") # 6
Doom_O.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter P 
# +--------------------------------+
Doom_P = []
Doom_P.append("       ") # Top, 0
Doom_P.append("______ ") # 1
Doom_P.append("| ___ \\") # 2
Doom_P.append("| |_/ /") # 3
Doom_P.append("|  __/ ") # 4
Doom_P.append("| |    ") # 5
Doom_P.append("\_|    ") # 6
Doom_P.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter Q 
# +--------------------------------+
Doom_Q = []
Doom_Q.append("       ") # Top, 0
Doom_Q.append(" _____ ") # 1
Doom_Q.append("|  _  |") # 2
Doom_Q.append("| | | |") # 3
Doom_Q.append("| | | |") # 4
Doom_Q.append("\ \/' /") # 5
Doom_Q.append(" \_/\_\\") # 6
Doom_Q.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter R 
# +--------------------------------+
Doom_R = []
Doom_R.append("       ") # Top, 0
Doom_R.append("______ ") # 1
Doom_R.append("| ___ \\") # 2
Doom_R.append("| |_/ /") # 3
Doom_R.append("|    / ") # 4
Doom_R.append("| |\ \ ") # 5
Doom_R.append("\_| \_|") # 6
Doom_R.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter S 
# +--------------------------------+
Doom_S = []
Doom_S.append("       ") # Top, 0
Doom_S.append(" _____ ") # 1
Doom_S.append("/  ___|") # 2
Doom_S.append("\ `--. ") # 3
Doom_S.append(" `--. \\") # 4
Doom_S.append("/\__/ /") # 5
Doom_S.append("\____/ ") # 6
Doom_S.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter T 
# +--------------------------------+
Doom_T = []
Doom_T.append("       ") # Top, 0
Doom_T.append(" _____ ") # 1
Doom_T.append("|_   _|") # 2
Doom_T.append("  | |  ") # 3
Doom_T.append("  | |  ") # 4
Doom_T.append("  | |  ") # 5
Doom_T.append("  \_/  ") # 6
Doom_T.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter U 
# +--------------------------------+
Doom_U = []
Doom_U.append("       ") # Top, 0
Doom_U.append(" _   _ ") # 1
Doom_U.append("| | | |") # 2
Doom_U.append("| | | |") # 3
Doom_U.append("| | | |") # 4
Doom_U.append("| |_| |") # 5
Doom_U.append(" \___/ ") # 6
Doom_U.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter V 
# +--------------------------------+
Doom_V = []
Doom_V.append("       ") # Top, 0
Doom_V.append(" _   _ ") # 1
Doom_V.append("| | | |") # 2
Doom_V.append("| | | |") # 3
Doom_V.append("| | | |") # 4
Doom_V.append("\ \_/ /") # 5
Doom_V.append(" \___/ ") # 6
Doom_V.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter W 
# +--------------------------------+
Doom_W = []
Doom_W.append("        ") # Top, 0
Doom_W.append(" _    _ ") # 1
Doom_W.append("| |  | |") # 2
Doom_W.append("| |  | |") # 3
Doom_W.append("| |/\| |") # 4
Doom_W.append("\  /\  /") # 5
Doom_W.append(" \/  \/ ") # 6
Doom_W.append("        ") # Bottom, 7

# +--------------------------------+
#   Letter X 
# +--------------------------------+
Doom_X = []
Doom_X.append("       ") # Top, 0
Doom_X.append("__   __") # 1
Doom_X.append("\ \ / /") # 2
Doom_X.append(" \ V / ") # 3
Doom_X.append(" /   \ ") # 4
Doom_X.append("/ /^\ \\") # 5
Doom_X.append("\/   \/") # 6
Doom_X.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter Y 
# +--------------------------------+
Doom_Y = []
Doom_Y.append("       ") # Top, 0
Doom_Y.append("__   __") # 1
Doom_Y.append("\ \ / /") # 2
Doom_Y.append(" \ V / ") # 3
Doom_Y.append("  \ /  ") # 4
Doom_Y.append("  | |  ") # 5
Doom_Y.append("  \_/  ") # 6
Doom_Y.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter Z 
# +--------------------------------+
Doom_Z = []
Doom_Z.append("       ") # Top, 0
Doom_Z.append(" ______") # 1
Doom_Z.append("|___  /") # 2
Doom_Z.append("   / / ") # 3
Doom_Z.append("  / /  ") # 4
Doom_Z.append("./ /___") # 5
Doom_Z.append("\_____/") # 6
Doom_Z.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter a 
# +--------------------------------+
Doom_a = []
Doom_a.append("       ") # Top, 0
Doom_a.append("       ") # 1
Doom_a.append("       ") # 2
Doom_a.append("  __ _ ") # 3
Doom_a.append(" / _` |") # 4
Doom_a.append("| (_| |") # 5
Doom_a.append(" \__,_|") # 6
Doom_a.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter b 
# +--------------------------------+
Doom_b = []
Doom_b.append("       ") # Top, 0
Doom_b.append(" _     ") # 1
Doom_b.append("| |    ") # 2
Doom_b.append("| |__  ") # 3
Doom_b.append("| '_ \ ") # 4
Doom_b.append("| |_) |") # 5
Doom_b.append("|_.__/ ") # 6
Doom_b.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter c 
# +--------------------------------+
Doom_c = []
Doom_c.append("      ") # Top, 0
Doom_c.append("      ") # 1
Doom_c.append("      ") # 2
Doom_c.append("  ___ ") # 3
Doom_c.append(" / __|") # 4
Doom_c.append("| (__ ") # 5
Doom_c.append(" \___|") # 6
Doom_c.append("      ") # Bottom, 7

# +--------------------------------+
#   Letter d 
# +--------------------------------+
Doom_d = []
Doom_d.append("       ") # Top, 0
Doom_d.append("     _ ") # 1
Doom_d.append("    | |") # 2
Doom_d.append("  __| |") # 3
Doom_d.append(" / _` |") # 4
Doom_d.append("| (_| |") # 5
Doom_d.append(" \__,_|") # 6
Doom_d.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter e 
# +--------------------------------+
Doom_e = []
Doom_e.append("      ") # Top, 0
Doom_e.append("      ") # 1
Doom_e.append("      ") # 2
Doom_e.append("  ___ ") # 3
Doom_e.append(" / _ \\") # 4
Doom_e.append("|  __/") # 5
Doom_e.append(" \___|") # 6
Doom_e.append("      ") # Bottom, 7

# +--------------------------------+
#   Letter f 
# +--------------------------------+
Doom_f = []
Doom_f.append("     ") # Top, 0
Doom_f.append("  __ ") # 1
Doom_f.append(" / _|") # 2
Doom_f.append("| |_ ") # 3
Doom_f.append("|  _|") # 4
Doom_f.append("| |  ") # 5
Doom_f.append("|_|  ") # 6
Doom_f.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter g 
# +--------------------------------+
Doom_g = []
Doom_g.append("       ") # Top, 0
Doom_g.append("  __ _ ") # 1
Doom_g.append(" / _` |") # 2
Doom_g.append("| (_| |") # 3
Doom_g.append(" \__, |") # 4
Doom_g.append("  __/ |") # 5
Doom_g.append(" |___/ ") # 6
Doom_g.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter h 
# +--------------------------------+
Doom_h = []
Doom_h.append("       ") # Top, 0
Doom_h.append(" _     ") # 1
Doom_h.append("| |    ") # 2
Doom_h.append("| |__  ") # 3
Doom_h.append("| '_ \ ") # 4
Doom_h.append("| | | |") # 5
Doom_h.append("|_| |_|") # 6
Doom_h.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter i 
# +--------------------------------+
Doom_i = []
Doom_i.append("   ") # Top, 0
Doom_i.append(" _ ") # 1
Doom_i.append("(_)") # 2
Doom_i.append(" _ ") # 3
Doom_i.append("| |") # 4
Doom_i.append("| |") # 5
Doom_i.append("|_|") # 6
Doom_i.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter j 
# +--------------------------------+
Doom_j = []
Doom_j.append("     ") # Top, 0
Doom_j.append("   _ ") # 1
Doom_j.append("  (_)") # 2
Doom_j.append("   _ ") # 3
Doom_j.append("  | |") # 4
Doom_j.append(" _/ |") # 5
Doom_j.append("|__/ ") # 6
Doom_j.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter k 
# +--------------------------------+
Doom_k = []
Doom_k.append("      ") # Top, 0
Doom_k.append(" _    ") # 1
Doom_k.append("| |   ") # 2
Doom_k.append("| | __") # 3
Doom_k.append("| |/ /") # 4
Doom_k.append("|   < ") # 5
Doom_k.append("|_|\_\\") # 6
Doom_k.append("      ") # Bottom, 7

# +--------------------------------+
#   Letter l 
# +--------------------------------+
Doom_l = []
Doom_l.append("   ") # Top, 0
Doom_l.append(" _ ") # 1
Doom_l.append("| |") # 2
Doom_l.append("| |") # 3
Doom_l.append("| |") # 4
Doom_l.append("| |") # 5
Doom_l.append("|_|") # 6
Doom_l.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter m 
# +--------------------------------+
Doom_m = []
Doom_m.append("           ") # Top, 0
Doom_m.append("           ") # 1
Doom_m.append("           ") # 2
Doom_m.append(" _ __ ___  ") # 3
Doom_m.append("| '_ ` _ \ ") # 4
Doom_m.append("| | | | | |") # 5
Doom_m.append("|_| |_| |_|") # 6
Doom_m.append("           ") # Bottom, 7

# +--------------------------------+
#   Letter n 
# +--------------------------------+
Doom_n = []
Doom_n.append("       ") # Top, 0
Doom_n.append("       ") # 1
Doom_n.append("       ") # 2
Doom_n.append(" _ __  ") # 3
Doom_n.append("| '_ \ ") # 4
Doom_n.append("| | | |") # 5
Doom_n.append("|_| |_|") # 6
Doom_n.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter ñ 
# +--------------------------------+
Doom_ñ = []
Doom_ñ.append("       ") # Top, 0
Doom_ñ.append("  /~~~/") # 1
Doom_ñ.append(" /~~~~/") # 2
Doom_ñ.append(" _ __  ") # 3
Doom_ñ.append("| '_ \ ") # 4
Doom_ñ.append("| | | |") # 5
Doom_ñ.append("|_| |_|") # 6
Doom_ñ.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter o 
# +--------------------------------+
Doom_o = []
Doom_o.append("       ") # Top, 0
Doom_o.append("       ") # 1
Doom_o.append("       ") # 2
Doom_o.append("  ___  ") # 3
Doom_o.append(" / _ \ ") # 4
Doom_o.append("| (_) |") # 5
Doom_o.append(" \___/ ") # 6
Doom_o.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter p 
# +--------------------------------+
Doom_p = []
Doom_p.append("       ") # Top, 0
Doom_p.append(" _ __  ") # 1
Doom_p.append("| '_ \ ") # 2
Doom_p.append("| |_) |") # 3
Doom_p.append("| .__/ ") # 4
Doom_p.append("| |    ") # 5
Doom_p.append("|_|    ") # 6
Doom_p.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter q 
# +--------------------------------+
Doom_q = []
Doom_q.append("       ") # Top, 0
Doom_q.append("  __ _ ") # 1
Doom_q.append(" / _` |") # 2
Doom_q.append("| (_| |") # 3
Doom_q.append(" \__, |") # 4
Doom_q.append("    | |") # 5
Doom_q.append("    |_|") # 6
Doom_q.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter r 
# +--------------------------------+
Doom_r = []
Doom_r.append("       ") # Top, 0
Doom_r.append("       ") # 1
Doom_r.append("       ") # 2
Doom_r.append("  _ __ ") # 3
Doom_r.append(" | '__|") # 4
Doom_r.append(" | |   ") # 5
Doom_r.append(" |_|   ") # 6
Doom_r.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter s 
# +--------------------------------+
Doom_s = []
Doom_s.append("     ") # Top, 0
Doom_s.append("     ") # 1
Doom_s.append("     ") # 2
Doom_s.append(" ___ ") # 3
Doom_s.append("/ __|") # 4
Doom_s.append("\__ \\") # 5
Doom_s.append("|___/") # 6
Doom_s.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter t 
# +--------------------------------+
Doom_t = []
Doom_t.append("     ") # Top, 0
Doom_t.append(" _   ") # 1
Doom_t.append("| |  ") # 2
Doom_t.append("| |_ ") # 3
Doom_t.append("| __|") # 4
Doom_t.append("| |_ ") # 5
Doom_t.append(" \__|") # 6
Doom_t.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter u 
# +--------------------------------+
Doom_u = []
Doom_u.append("       ") # Top, 0
Doom_u.append("       ") # 1
Doom_u.append("       ") # 2
Doom_u.append(" _   _ ") # 3
Doom_u.append("| | | |") # 4
Doom_u.append("| |_| |") # 5
Doom_u.append(" \__,_|") # 6
Doom_u.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter v 
# +--------------------------------+
Doom_v = []
Doom_v.append("       ") # Top, 0
Doom_v.append("       ") # 1
Doom_v.append("       ") # 2
Doom_v.append("__   __") # 3
Doom_v.append("\ \ / /") # 4
Doom_v.append(" \ V / ") # 5
Doom_v.append("  \_/  ") # 6
Doom_v.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter w 
# +--------------------------------+
Doom_w = []
Doom_w.append("          ") # Top, 0
Doom_w.append("          ") # 1
Doom_w.append("          ") # 2
Doom_w.append("__      __") # 3
Doom_w.append("\ \ /\ / /") # 4
Doom_w.append(" \ V  V / ") # 5
Doom_w.append("  \_/\_/  ") # 6
Doom_w.append("          ") # Bottom, 7

# +--------------------------------+
#   Letter x 
# +--------------------------------+
Doom_x = []
Doom_x.append("      ") # Top, 0
Doom_x.append("      ") # 1
Doom_x.append("      ") # 2
Doom_x.append("__  __") # 3
Doom_x.append("\ \/ /") # 4
Doom_x.append(" >  < ") # 5
Doom_x.append("/_/\_\\") # 6
Doom_x.append("      ") # Bottom, 7

# +--------------------------------+
#   Letter y 
# +--------------------------------+
Doom_y = []
Doom_y.append("      ") # Top, 0
Doom_y.append("_   _ ") # 1
Doom_y.append(" | | |") # 2
Doom_y.append(" |_| |") # 3
Doom_y.append("\__, |") # 4
Doom_y.append(" __/ |") # 5
Doom_y.append("|___/ ") # 6
Doom_y.append("      ") # Bottom, 7

# +--------------------------------+
#   Letter z 
# +--------------------------------+
Doom_z = []
Doom_z.append("     ") # Top, 0
Doom_z.append("     ") # 1
Doom_z.append("     ") # 2
Doom_z.append(" ____") # 3
Doom_z.append("|_  /") # 4
Doom_z.append(" / / ") # 5
Doom_z.append("/___|") # 6
Doom_z.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter backtick 
# +--------------------------------+
Doom_backtick = []
Doom_backtick.append("   ") # Top, 0
Doom_backtick.append(" _ ") # 1
Doom_backtick.append("( )") # 2
Doom_backtick.append(" \|") # 3
Doom_backtick.append("   ") # 4
Doom_backtick.append("   ") # 5
Doom_backtick.append("   ") # 6
Doom_backtick.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter one 
# +--------------------------------+
Doom_one = []
Doom_one.append("     ") # Top, 0
Doom_one.append(" __  ") # 1
Doom_one.append("/  | ") # 2
Doom_one.append("`| | ") # 3
Doom_one.append(" | | ") # 4
Doom_one.append("_| |_") # 5
Doom_one.append("\___/") # 6
Doom_one.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter two 
# +--------------------------------+
Doom_two = []
Doom_two.append("       ") # Top, 0
Doom_two.append(" _____ ") # 1
Doom_two.append("/ __  \\") # 2
Doom_two.append("`' / /'") # 3
Doom_two.append("  / /  ") # 4
Doom_two.append("./ /___") # 5
Doom_two.append("\_____/") # 6
Doom_two.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter three 
# +--------------------------------+
Doom_three = []
Doom_three.append("       ") # Top, 0
Doom_three.append(" _____ ") # 1
Doom_three.append("|____ |") # 2
Doom_three.append("    / /") # 3
Doom_three.append("    \ \\") # 4
Doom_three.append(".___/ /") # 5
Doom_three.append("\____/ ") # 6
Doom_three.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter four 
# +--------------------------------+
Doom_four = []
Doom_four.append("       ") # Top, 0
Doom_four.append("   ___ ") # 1
Doom_four.append("  /   |") # 2
Doom_four.append(" / /| |") # 3
Doom_four.append("/ /_| |") # 4
Doom_four.append("\___  |") # 5
Doom_four.append("    |_/") # 6
Doom_four.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter five 
# +--------------------------------+
Doom_five = []
Doom_five.append("       ") # Top, 0
Doom_five.append(" _____ ") # 1
Doom_five.append("|  ___|") # 2
Doom_five.append("|___ \ ") # 3
Doom_five.append("    \ \\") # 4
Doom_five.append("/\__/ /") # 5
Doom_five.append("\____/ ") # 6
Doom_five.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter six 
# +--------------------------------+
Doom_six = []
Doom_six.append("       ") # Top, 0
Doom_six.append("  ____ ") # 1
Doom_six.append(" / ___|") # 2
Doom_six.append("/ /___ ") # 3
Doom_six.append("| ___ \\") # 4
Doom_six.append("| \_/ |") # 5
Doom_six.append("\_____/") # 6
Doom_six.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter seven 
# +--------------------------------+
Doom_seven = []
Doom_seven.append("       ") # Top, 0
Doom_seven.append(" ______") # 1
Doom_seven.append("|___  /") # 2
Doom_seven.append("   / / ") # 3
Doom_seven.append("  / /  ") # 4
Doom_seven.append("./ /   ") # 5
Doom_seven.append("\_/    ") # 6
Doom_seven.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter eight 
# +--------------------------------+
Doom_eight = []
Doom_eight.append("       ") # Top, 0
Doom_eight.append(" _____ ") # 1
Doom_eight.append("|  _  |") # 2
Doom_eight.append(" \ V / ") # 3
Doom_eight.append(" / _ \ ") # 4
Doom_eight.append("| |_| |") # 5
Doom_eight.append("\_____/") # 6
Doom_eight.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter nine 
# +--------------------------------+
Doom_nine = []
Doom_nine.append("       ") # Top, 0
Doom_nine.append(" _____ ") # 1
Doom_nine.append("|  _  |") # 2
Doom_nine.append("| |_| |") # 3
Doom_nine.append("\____ |") # 4
Doom_nine.append(".___/ /") # 5
Doom_nine.append("\____/ ") # 6
Doom_nine.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter zero 
# +--------------------------------+
Doom_zero = []
Doom_zero.append("       ") # Top, 0
Doom_zero.append(" _____ ") # 1
Doom_zero.append("|  _  |") # 2
Doom_zero.append("| |/' |") # 3
Doom_zero.append("|  /| |") # 4
Doom_zero.append("\ |_/ /") # 5
Doom_zero.append(" \___/ ") # 6
Doom_zero.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter minus 
# +--------------------------------+
Doom_minus = []
Doom_minus.append("        ") # Top, 0
Doom_minus.append("        ") # 1
Doom_minus.append("        ") # 2
Doom_minus.append(" ______ ") # 3
Doom_minus.append("|______|") # 4
Doom_minus.append("        ") # 5
Doom_minus.append("        ") # 6
Doom_minus.append("        ") # Bottom, 7

# +--------------------------------+
#   Letter equal 
# +--------------------------------+
Doom_equal = []
Doom_equal.append("        ") # Top, 0
Doom_equal.append("        ") # 1
Doom_equal.append(" ______ ") # 2
Doom_equal.append("|______|") # 3
Doom_equal.append(" ______ ") # 4
Doom_equal.append("|______|") # 5
Doom_equal.append("        ") # 6
Doom_equal.append("        ") # Bottom, 7

# +--------------------------------+
#   Letter backward_slash 
# +--------------------------------+
Doom_backward_slash = []
Doom_backward_slash.append("       ") # Top, 0
Doom_backward_slash.append("__     ") # 1
Doom_backward_slash.append("\ \    ") # 2
Doom_backward_slash.append(" \ \   ") # 3
Doom_backward_slash.append("  \ \  ") # 4
Doom_backward_slash.append("   \ \ ") # 5
Doom_backward_slash.append("    \_\\") # 6
Doom_backward_slash.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter open_bracket 
# +--------------------------------+
Doom_open_bracket = []
Doom_open_bracket.append(" ___ ") # Top, 0
Doom_open_bracket.append("|  _|") # 1
Doom_open_bracket.append("| |  ") # 2
Doom_open_bracket.append("| |  ") # 3
Doom_open_bracket.append("| |  ") # 4
Doom_open_bracket.append("| |_ ") # 5
Doom_open_bracket.append("|___|") # 6
Doom_open_bracket.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter closed_bracket 
# +--------------------------------+
Doom_closed_bracket = []
Doom_closed_bracket.append(" ___ ") # Top, 0
Doom_closed_bracket.append("|_  |") # 1
Doom_closed_bracket.append("  | |") # 2
Doom_closed_bracket.append("  | |") # 3
Doom_closed_bracket.append("  | |") # 4
Doom_closed_bracket.append(" _| |") # 5
Doom_closed_bracket.append("|___|") # 6
Doom_closed_bracket.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter semicolon 
# +--------------------------------+
Doom_semicolon = []
Doom_semicolon.append("   ") # Top, 0
Doom_semicolon.append(" _ ") # 1
Doom_semicolon.append("(_)") # 2
Doom_semicolon.append("   ") # 3
Doom_semicolon.append(" _ ") # 4
Doom_semicolon.append("( )") # 5
Doom_semicolon.append("|/ ") # 6
Doom_semicolon.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter apostrophe 
# +--------------------------------+
Doom_apostrophe = []
Doom_apostrophe.append(" _ ") # Top, 0
Doom_apostrophe.append("( )") # 1
Doom_apostrophe.append("|/ ") # 2
Doom_apostrophe.append("   ") # 3
Doom_apostrophe.append("   ") # 4
Doom_apostrophe.append("   ") # 5
Doom_apostrophe.append("   ") # 6
Doom_apostrophe.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter comma 
# +--------------------------------+
Doom_comma = []
Doom_comma.append("   ") # Top, 0
Doom_comma.append("   ") # 1
Doom_comma.append("   ") # 2
Doom_comma.append("   ") # 3
Doom_comma.append(" _ ") # 4
Doom_comma.append("( )") # 5
Doom_comma.append("|/ ") # 6
Doom_comma.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter period 
# +--------------------------------+
Doom_period = []
Doom_period.append("   ") # Top, 0
Doom_period.append("   ") # 1
Doom_period.append("   ") # 2
Doom_period.append("   ") # 3
Doom_period.append(" _ ") # 4
Doom_period.append("(_)") # 5
Doom_period.append("   ") # 6
Doom_period.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter forward_slash 
# +--------------------------------+
Doom_forward_slash = []
Doom_forward_slash.append("     __") # Top, 0
Doom_forward_slash.append("    / /") # 1
Doom_forward_slash.append("   / / ") # 2
Doom_forward_slash.append("  / /  ") # 3
Doom_forward_slash.append(" / /   ") # 4
Doom_forward_slash.append("/_/    ") # 5
Doom_forward_slash.append("       ") # 6
Doom_forward_slash.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter tilde 
# +--------------------------------+
Doom_tilde = []
Doom_tilde.append("     ") # Top, 0
Doom_tilde.append(" /\/|") # 1
Doom_tilde.append("|/\/ ") # 2
Doom_tilde.append("     ") # 3
Doom_tilde.append("     ") # 4
Doom_tilde.append("     ") # 5
Doom_tilde.append("     ") # 6
Doom_tilde.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter exclamation 
# +--------------------------------+
Doom_exclamation = []
Doom_exclamation.append("   ") # Top, 0
Doom_exclamation.append(" _ ") # 1
Doom_exclamation.append("| |") # 2
Doom_exclamation.append("| |") # 3
Doom_exclamation.append("| |") # 4
Doom_exclamation.append("|_|") # 5
Doom_exclamation.append("(_)") # 6
Doom_exclamation.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter arroba 
# +--------------------------------+
Doom_arroba = []
Doom_arroba.append("         ") # Top, 0
Doom_arroba.append("         ") # 1
Doom_arroba.append("   ____  ") # 2
Doom_arroba.append("  / __ \ ") # 3
Doom_arroba.append(" / / _` |") # 4
Doom_arroba.append("| | (_| |") # 5
Doom_arroba.append(" \ \__,_|") # 6
Doom_arroba.append("  \____/ ") # Bottom, 7

# +--------------------------------+
#   Letter pound 
# +--------------------------------+
Doom_pound = []
Doom_pound.append("          ") # Top, 0
Doom_pound.append("   _  _   ") # 1
Doom_pound.append(" _| || |_ ") # 2
Doom_pound.append("|_  __  _|") # 3
Doom_pound.append(" _| || |_ ") # 4
Doom_pound.append("|_  __  _|") # 5
Doom_pound.append("  |_||_|  ") # 6
Doom_pound.append("          ") # Bottom, 7
# +--------------------------------+
#   Letter dollar 
# +--------------------------------+
Doom_dollar = []
Doom_dollar.append("     ") # Top, 0
Doom_dollar.append("  _  ") # 1
Doom_dollar.append(" | | ") # 2
Doom_dollar.append("/ __)") # 3
Doom_dollar.append("\__ \\") # 4
Doom_dollar.append("(   /") # 5
Doom_dollar.append(" |_| ") # 6
Doom_dollar.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter percent 
# +--------------------------------+
Doom_percent = []
Doom_percent.append("       ") # Top, 0
Doom_percent.append(" _   __") # 1
Doom_percent.append("(_) / /") # 2
Doom_percent.append("   / / ") # 3
Doom_percent.append("  / /  ") # 4
Doom_percent.append(" / / _ ") # 5
Doom_percent.append("/_/ (_)") # 6
Doom_percent.append("       ") # Bottom, 7

# +--------------------------------+
#   Letter caret 
# +--------------------------------+
Doom_caret = []
Doom_caret.append("    ") # Top, 0
Doom_caret.append(" /\ ") # 1
Doom_caret.append("|/\|") # 2
Doom_caret.append("    ") # 3
Doom_caret.append("    ") # 4
Doom_caret.append("    ") # 5
Doom_caret.append("    ") # 6
Doom_caret.append("    ") # Bottom, 7

# +--------------------------------+
#   Letter ampersand 
# +--------------------------------+
Doom_ampersand = []
Doom_ampersand.append("        ") # Top, 0
Doom_ampersand.append("        ") # 1
Doom_ampersand.append("  ___   ") # 2
Doom_ampersand.append(" ( _ )  ") # 3
Doom_ampersand.append(" / _ \/\\") # 4
Doom_ampersand.append("| (_>  <") # 5
Doom_ampersand.append(" \___/\/") # 6
Doom_ampersand.append("        ") # Bottom, 7

# +--------------------------------+
#   Letter asterisk 
# +--------------------------------+
Doom_asterisk = []
Doom_asterisk.append("         ") # Top, 0
Doom_asterisk.append("    _    ") # 1
Doom_asterisk.append(" /\| |/\ ") # 2
Doom_asterisk.append(" \ ` ' / ") # 3
Doom_asterisk.append("|_     _|") # 4
Doom_asterisk.append(" / , . \ ") # 5
Doom_asterisk.append(" \/|_|\/ ") # 6
Doom_asterisk.append("         ") # Bottom, 7

# +--------------------------------+
#   Letter open_parenthesis 
# +--------------------------------+
Doom_open_parenthesis = []
Doom_open_parenthesis.append("     ") # Top, 0
Doom_open_parenthesis.append("   __") # 1
Doom_open_parenthesis.append("  / /") # 2
Doom_open_parenthesis.append(" | | ") # 3
Doom_open_parenthesis.append(" | | ") # 4
Doom_open_parenthesis.append(" | | ") # 5
Doom_open_parenthesis.append("  \_\\") # 6
Doom_open_parenthesis.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter closed_parenthesis 
# +--------------------------------+
Doom_closed_parenthesis = []
Doom_closed_parenthesis.append("    ") # Top, 0
Doom_closed_parenthesis.append("__  ") # 1
Doom_closed_parenthesis.append("\ \ ") # 2
Doom_closed_parenthesis.append(" | |") # 3
Doom_closed_parenthesis.append(" | |") # 4
Doom_closed_parenthesis.append(" | |") # 5
Doom_closed_parenthesis.append("/_/ ") # 6
Doom_closed_parenthesis.append("    ") # Bottom, 7

# +--------------------------------+
#   Letter underscore 
# +--------------------------------+
Doom_underscore = []
Doom_underscore.append("        ") # Top, 0
Doom_underscore.append("        ") # 1
Doom_underscore.append("        ") # 2
Doom_underscore.append("        ") # 3
Doom_underscore.append("        ") # 4
Doom_underscore.append(" ______ ") # 5
Doom_underscore.append("|______|") # 6
Doom_underscore.append("        ") # Bottom, 7

# +--------------------------------+
#   Letter plus 
# +--------------------------------+
Doom_plus = []
Doom_plus.append("        ") # Top, 0
Doom_plus.append("        ") # 1
Doom_plus.append("    _   ") # 2
Doom_plus.append("  _| |_ ") # 3
Doom_plus.append(" |_   _|") # 4
Doom_plus.append("   |_|  ") # 5
Doom_plus.append("        ") # 6
Doom_plus.append("        ") # Bottom, 7

# +--------------------------------+
#   Letter pipe 
# +--------------------------------+
Doom_pipe = []
Doom_pipe.append("   ") # Top, 0
Doom_pipe.append(" _ ") # 1
Doom_pipe.append("| |") # 2
Doom_pipe.append("| |") # 3
Doom_pipe.append("| |") # 4
Doom_pipe.append("| |") # 5
Doom_pipe.append("|_|") # 6
Doom_pipe.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter open_curly 
# +--------------------------------+
Doom_open_curly = []
Doom_open_curly.append("   __") # Top, 0
Doom_open_curly.append("  / /") # 1
Doom_open_curly.append(" | | ") # 2
Doom_open_curly.append("/ /  ") # 3
Doom_open_curly.append("\ \  ") # 4
Doom_open_curly.append(" | | ") # 5
Doom_open_curly.append("  \_\\") # 6
Doom_open_curly.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter closed_curly 
# +--------------------------------+
Doom_closed_curly = []
Doom_closed_curly.append("__   ") # Top, 0
Doom_closed_curly.append("\ \  ") # 1
Doom_closed_curly.append(" | | ") # 2
Doom_closed_curly.append("  \ \\") # 3
Doom_closed_curly.append("  / /") # 4
Doom_closed_curly.append(" | | ") # 5
Doom_closed_curly.append("/_/  ") # 6
Doom_closed_curly.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter colon 
# +--------------------------------+
Doom_colon = []
Doom_colon.append("   ") # Top, 0
Doom_colon.append(" _ ") # 1
Doom_colon.append("(_)") # 2
Doom_colon.append("   ") # 3
Doom_colon.append(" _ ") # 4
Doom_colon.append("(_)") # 5
Doom_colon.append("   ") # 6
Doom_colon.append("   ") # Bottom, 7

# +--------------------------------+
#   Letter quotation 
# +--------------------------------+
Doom_quotation = []
Doom_quotation.append("     ") # Top, 0
Doom_quotation.append(" _ _ ") # 1
Doom_quotation.append("( | )") # 2
Doom_quotation.append(" V V ") # 3
Doom_quotation.append("     ") # 4
Doom_quotation.append("     ") # 5
Doom_quotation.append("     ") # 6
Doom_quotation.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter less_than 
# +--------------------------------+
Doom_less_than = []
Doom_less_than.append("     ") # Top, 0
Doom_less_than.append("   __") # 1
Doom_less_than.append("  / /") # 2
Doom_less_than.append(" / / ") # 3
Doom_less_than.append("< <  ") # 4
Doom_less_than.append(" \ \ ") # 5
Doom_less_than.append("  \_\\") # 6
Doom_less_than.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter greater_than 
# +--------------------------------+
Doom_greater_than = []
Doom_greater_than.append("     ") # Top, 0
Doom_greater_than.append("__   ") # 1
Doom_greater_than.append("\ \  ") # 2
Doom_greater_than.append(" \ \ ") # 3
Doom_greater_than.append("  > >") # 4
Doom_greater_than.append(" / / ") # 5
Doom_greater_than.append("/_/  ") # 6
Doom_greater_than.append("     ") # Bottom, 7

# +--------------------------------+
#   Letter question 
# +--------------------------------+
Doom_question = []
Doom_question.append("      ") # Top, 0
Doom_question.append(" ___  ") # 1
Doom_question.append("|__ \ ") # 2
Doom_question.append("   ) |") # 3
Doom_question.append("  / / ") # 4
Doom_question.append(" |_|  ") # 5
Doom_question.append(" (_)  ") # 6
Doom_question.append("      ") # Bottom, 7

# +--------------------------------+
#   Letter space 
# +--------------------------------+
Doom_space = []
Doom_space.append(" ") # Top, 0
Doom_space.append(" ") # 1
Doom_space.append(" ") # 2
Doom_space.append(" ") # 3
Doom_space.append(" ") # 4
Doom_space.append(" ") # 5
Doom_space.append(" ") # 6
Doom_space.append(" ") # Bottom, 7

# +--------------------------------+
#   Letter NA 
# +--------------------------------+
Doom_NA = []
Doom_NA.append("N/A") # Top, 0
Doom_NA.append("N/A") # 1
Doom_NA.append("N/A") # 2
Doom_NA.append("N/A") # 3
Doom_NA.append("N/A") # 4
Doom_NA.append("N/A") # 5
Doom_NA.append("N/A") # 6
Doom_NA.append("N/A") # Bottom, 7

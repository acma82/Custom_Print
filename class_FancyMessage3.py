...
def message_with_note(obj:fp.FancyMessage, note_msg:str="Warning", bg_note:int=231, fg_note:int=0,\
   ...
   if (align_note == fp.Align.LEFT):
      print(f"{crs.move(qty=n_lines+obj.bottom_lines, direction=fp.Move.UP)}{fp.set_font(1,bg_note,fg_note)}{note}")
   elif (align_note == fp.Align.CENTER):
      print(f"{crs.move(qty=n_lines+msg.bottom_lines, direction=fp.Move.UP)}{crs.move(qty=1,direction=fp.Move.RIGHT)}{fp.set_font(1,bg_note,fg_note)}{note}")
   elif (align_note == fp.Align.RIGHT):
      print(f"{crs.move(qty=n_lines+msg.bottom_lines, direction=fp.Move.UP)}{crs.move(qty=2,direction=fp.Move.RIGHT)}{fp.set_font(1,bg_note,fg_note)}{note}")
   else:
      print(f"{crs.move(qty=n_lines+obj.bottom_lines, direction=fp.Move.UP)}{fp.set_font(1,bg_note,fg_note)}{note}")
...
msg.bottom_lines = 1
msg.right_indent = 2
message_with_note(obj=msg, note_msg=note, bg_note=90, fg_note=231, align_note=fp.Align.CENTER, body_msg=warning_body, fg_body_msg=21, bg_body=231)
